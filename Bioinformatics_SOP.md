...
1.  Study naming for isolates and sequences. Separate sections with dashes "-". 
    1. Prefix: "GSAR"
    2. Country code 
        * NG: Nigeria
        * PE: Peru
...
2.	MinKNOW basecalling and demultiplexing:
    ...
    b.	Run limit: 72 hours
    c.	Pore scan frequency: 1.5 hrs
    d.	Basecalling: Super-accurate basecalling 400bps
    e.	Modified basecalling: Off
    f.	Trim barcodes: On
    g.	Min Q Score: 10
    h.	BAM file output: Off
...
4.  Create barcode list to REDCap as Excel or tab-separated text file. Must have at least two columns with first two columns being, in order, "barcode" and "id". Additional columns can be included, if wanted. 
...
reads=`realpath /path/to/GSAR-XX-YYYY-MM-DD/no_sample_id/<run_code>/GSAR-ZA-KP-01238.fastq.gz` ## Example path to corresponding read file 
mkdir -p $iso
cd $iso
```
<br>

7. Activate the first conda environment and set the number of threads to be used

```Shell
conda activate genosar_pt1_env
threads=8 ## Set to number of threads available on your system
```
<br>
...
Use BUSCO to assess completeness and contamination levels.
 ...
 > :bulb: To avoid having to download the database file (147 Mb) every time you run this command, create a `busco_database` directory. In the database directory run the following command: `busco --download enterobacteriaceae_odb12`. The database will be downloaded into `busco_downloads/lineages/enterobacteriaceae_odb12/`. Give the path to the downloaded database to the `-l` option in the command below.

```Shell
busco -i ${iso}.fasta \
    -m genome \
    -l path/to/busco_database/busco_downloads/lineages/enterobacteriaceae_odb12 \
    --offline \
    -o busco \
    -c $threads
```
...
We'll use a custom Gammaproteobacteria database generated from NCBI reference sequences and override the default threshold of 5 to improve the specificity of the species identification.
...
sourmash lca classify \
    --db /path/to/gammaproteobacteria.lca.json.gz \
    --threshold 10 \
...
```Shell
mlst_profiler \
    -o KP \ ## or "EC" depending on the species identified in the previous step
...
amrfinder \
    -n ${iso}.fasta \
    --name $iso \
    -O Klebsiella_pneumoniae \ # or "Escherichia" depending on the species
    -o amrfinder.txt 
```